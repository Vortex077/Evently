...
export default function PhotosUploader({addedPhotos,onChange}){
    ...
    async function addPhotoByLink(ev){
        ev.preventDefault();
        try {
            // Make a POST request to the server with the photo link
    ...
    return(
        ...
        <div className="flex pr-4 gap-2">
                    <input type="text" value={photoLink} onChange={ev=> setPhotoLink(ev.target.value)} placeholder={"add using a link.....jpg"} className=" border w-9/12 border-blue-300 rounded-2xl my-1 px-3 py-2"/>
                    <button onClick={addPhotoByLink} className=" w-3/12 bg-lightsky rounded-2xl">
                        Add&nbsp;photo
                    </button>
                    </div>
                    <div className=" mt-2 grid gap-2 grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
                        {addedPhotos.length > 0 && addedPhotos.map(link => (
                             ...
                             <img className="rounded-2xl w-full object-cover" src={`http://localhost:4000/uploads/${link}`} alt="Uploaded"></img>
